...
-- Función para escribir una línea al archivo
-- Función para escribir una línea al archivo
escribirArchivo :: String -> IO ()
escribirArchivo linea = withFile "arbol.txt" AppendMode $ \handle ->
    hPutStrLn handle linea



-- Función para pegar en la última línea
pegarEnUltimaLinea :: FilePath -> String -> IO ()
pegarEnUltimaLinea archivo contenidoNuevo = withFile archivo ReadWriteMode $ \handle -> do
...
-- Función para iterar mientras se cumpla una condición
while :: IO Bool -> IO () -> IO ()
...
-- Función para realizar una operación en el árbol
-- Función para operar sobre los tokens
-- Función para operar sobre los tokens
...
limpiarArchivo archivo = writeFile archivo ""

--cerramos la funcion de limpiar archivo



-- FIN DE LA FUNCION QUE GENERAMOS PARA EL ARBOL
...
main = do
  ...
  case estructuraValida of
    ...
    Nothing -> do
      ...
      derivacionA
      putStrLn "Tabla generada en arbol.txt"
